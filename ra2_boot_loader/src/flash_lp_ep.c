...
//#include "crc.h"
//#include "utils.h"
 uint32_t read_array[FLASH_CF_BLOCK_SIZE/4];
/*******************************************************************************************************************//**
 * @addtogroup flash_lp_ep
 * @{
 **********************************************************************************************************************/
#define START_IMAGE_1_BLOCK          3   // IMAGE_1 STATS AT 0x1800 each block is 2048 bytes
#define START_IMAGE_2_BLOCK          34  // IMAGE_1 STATS AT 0x1800 each block is 2048 bytes
#define END_IMAGE_2_BLOCK            64
...
fsp_err_t OverWrite_image_1(void)
{
    fsp_err_t err;
    uint8_t block_num;
    ...
    flash_lp_init();
    err = R_FLASH_LP_Erase (&g_flash0_ctrl, FLASH_CF_BLOCK(START_IMAGE_1_BLOCK), MAX_BLOCKS_IN_IMAGE);
    if (FSP_SUCCESS != err)
    {
             return err;
    }
    // out parameter for BlankCheck()
    uint8_t bInd=0;
    for (block_num = START_IMAGE_1_BLOCK; block_num < MAX_BLOCKS_IN_IMAGE+START_IMAGE_1_BLOCK;block_num++)
    {

        err = R_FLASH_LP_BlankCheck (&g_flash0_ctrl, FLASH_CF_BLOCK(block_num), FLASH_CF_BLOCK_SIZE, &blankCheck);
         /* error handle */
         if (FSP_SUCCESS != err)
         {
             return err;
         }
         memcpy ((uint8_t*) read_array, (unsigned char*) (__IO uint32_t*) FLASH_CF_BLOCK(bInd +START_IMAGE_2_BLOCK) ,FLASH_CF_BLOCK_SIZE);
         err =  R_FLASH_LP_Write (&g_flash0_ctrl, read_array,FLASH_CF_BLOCK(block_num), FLASH_CF_BLOCK_SIZE);
         bInd++;
         if (FSP_SUCCESS != err)
         {
               return err;
